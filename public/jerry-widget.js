...
'.jerry-widget-attach{background:none;border:none;font-size:18px;cursor:pointer;padding:4px 6px;opacity:.5;flex-shrink:0;line-height:1}' +
  ...
  function renderMessage(target, text) {
    text = String(text || '')
      .replace(/(?:INTAKE_COMPLETE|LISTING_DRAFT|LISTING_SAVE|LISTING_INTAKE_STATE):\{[\s\S]*?\}\s*$/m, '')
      .trim();
...
// FAA N-number lookup via our proxy
    ...
    var response = await fetch(apiUrl, {
      ...
      if (isStreaming) {
        ...
        streamRow.className = 'jerry-widget-row assistant';
        var streamMsg = document.createElement('div');
        streamMsg.className = 'jerry-widget-msg';
        streamRow.appendChild(streamMsg);
        ...
        while (true) {
          ...
          if (chunk.done) break;
          sBuffer += decoder.decode(chunk.value, { stream: true });
          var sLines = sBuffer.split('\n');
          sBuffer = sLines.pop() || '';

          for (var si = 0; si < sLines.length; si++) {
            if (sLines[si].startsWith('data: ')) {
              try {
        ...
        streamRow.remove(); // Will be re-added by normal message flow below
      } else {
