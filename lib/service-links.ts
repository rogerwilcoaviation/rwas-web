...
const links: SeoServiceLink[] = [
  {
    ...
    description:
      'Certified Garmin dealer support for panel planning, installation, testing, and documentation.',
  },
  {
    href: '/services/g3x-touch-installation',
    label: 'G3X Touch installation',
    description:
      'Glass-panel planning, EIS, ADAHRS, fabrication, configuration, and documentation.',
  },
  {
    href: '/services/gtn-xi-navigator-installation',
    label: 'GTN Xi navigator installation',
    description:
      'GTN 650Xi and GTN 750Xi planning, antenna review, integration, testing, and documentation.',
  },
  {
    href: '/services/gfc-500-autopilot-installation',
    label: 'GFC 500 autopilot installation',
    description:
      'Eligibility review, servo placement, integration, functional testing, and return-to-service paperwork.',
  },
  {
    href: '/services/ads-b-installation',
    label: 'ADS-B Out installation',
    description:
      'Transponder upgrades, GPS source review, antenna planning, configuration, and compliance documentation.',
  },
  {
  ...
  },
  {
    ...
    description:
      'Logbook review, physical inspection, avionics review, NDT coordination, and buyer decision support.',
  },
  {
    href: '/services/ndt-inspection',
    label: 'Aircraft NDT inspection',
    description:
      'Eddy current, dye penetrant, magnetic particle, ultrasound, visual, and Rockwell hardness testing.',
  },
  {
    href: '/services/fiber-laser-fabrication',
    label: 'Fiber laser fabrication',
    description:
      'Aircraft panel cutting, laser welding, powder coating, UV printing, bracket work, and fabrication.',
  },
  {
  ...
  },
  {
    ...
    description:
      'RWAS-designed Piper rigging reference tools for PA-series flight-control setup work.',
  },
];
...
export function serviceLinksForProduct(input: {
  ...
  if (
    /garmin|g3x|gtn|gfc|ads-?b|transponder|navigator|autopilot|avionics|panel|gdu|g5|gi\s?275|gtx|gma/.test(
      haystack,
    )
  )
  ...
  if (/maintenance|annual|100-hour|inspection|aog/.test(haystack))
    selected.push(links[5]);
  if (/pre-?buy|buyer/.test(haystack)) selected.push(links[6]);
  if (/ndt|eddy|penetrant|magnetic|ultrasound|rockwell/.test(haystack))
    selected.push(links[7]);
  if (
    /fabrication|fiber|laser|panel|bracket|powder|uv print|sheet metal/.test(
      haystack,
    )
  )
    selected.push(links[8]);
  if (/rotax|912|914|915|916/.test(haystack)) selected.push(links[9]);
  if (
    /papa-alpha|papa alpha|pa-28|pa-30|pa-31|pa-32|pa-34|pa-36|pa-44|rigging|bell crank|stabilator|rudder|aileron|flap/.test(
      haystack,
    )
  )
    selected.push(links[10]);

  return unique(selected).slice(0, 4);
}

export function serviceLinksForCollection(
  handle: string,
  title?: string | null,
) {
  ...
  if (
    /garmin-dealer-install|avionics-certified|avionics-experimental|garmin|avionics/.test(
      key,
    )
  ) {
  ...
  if (/service|parts|maintenance/.test(key))
    selected.push(links[5], links[7], links[8]);
...
export function serviceLinksForBlogArticle(input: {
  ...
  if (
    /avionics|panel|g3x|g500|txi|gtn|gfc|ads-?b|audio-panel|g1000|perspective|stc/.test(
      haystack,
    )
  )
    selected.push(links[0]);
  ...
  if (
    /maintenance|repair-station|service-bulletin|airworthiness|electroair|electronic-ignition|lycoming|continental/.test(
      haystack,
    )
  )
    selected.push(links[5]);
  ...
  if (/ndt|eddy|penetrant|magnetic|ultrasound|rockwell/.test(haystack))
    selected.push(links[7]);
  if (/fabrication|fiber|laser|sheet-metal|panel-cutting/.test(haystack))
    selected.push(links[8]);
  if (/rotax/.test(haystack)) selected.push(links[9]);
  if (
    /papa-alpha|papa alpha|rigging|pa-28|pa-32|stabilator|bell-crank/.test(
      haystack,
    )
  )
    selected.push(links[10]);
