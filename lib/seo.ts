...
export function collectionMetaDescription(collection: {
  ...
  if (handle.includes('garmin')) {
    return truncateMeta(
      `Browse ${title} from Roger Wilco Aviation Services, an authorized Garmin dealer and FAA Part 145 repair station in Yankton, SD.`,
    );
  }
  if (handle.includes('sale')) {
    return truncateMeta(
      'Browse current RWAS sale items, Garmin pilot gear, avionics accessories, and shop-supported aviation products from Yankton, SD.',
    );
  }
  return truncateMeta(
...
export function productMetaDescription(product: {
  ...
  return truncateMeta(
    `${title} from ${vendor}, available through RWAS in Yankton, SD. ${type} support from an FAA Part 145 repair station and authorized Garmin dealer.${skuText}`,
  );
}
