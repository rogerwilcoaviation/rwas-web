...
export async function applyPrivateCartPricing(
  ...
  for (const old of lines) {
    ...
    const e = entries.find((e) => e.variantId === l.merchandise?.id);
    if (!e || !active) continue;
    const currentTags = (l.merchandise.product?.tags || []).map((t) =>
      t.toLowerCase(),
    );
    if (
      l.merchandise.sku !== e.sku ||
      l.merchandise.product?.id !== e.productId ||
      l.merchandise.product?.handle !== e.handle ||
      l.merchandise.price?.currencyCode !== 'USD' ||
      money(l.merchandise.price.amount) !== money(e.publicPrice) ||
      !currentTags.includes('otc-eligible') ||
      currentTags.some((t) =>
        ['garmin-dealer-only', 'otc-disabled', 'stock-check-required'].includes(
          t,
        ),
