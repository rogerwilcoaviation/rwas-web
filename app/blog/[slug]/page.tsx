    ...
    .replace(
      ...
      (_match, label: string, href: string) => {
        ...
        const attributedHref =
          isAxisPlanner && !href.includes('?')
            ? `${href}?source=article`
            : href;
        return `<a href="${attributedHref}"${className}${inlineStyle}${isInternal ? '' : ' target="_blank" rel="noopener noreferrer"'}>${label}${arrow}</a>`;
      },
    );
}
