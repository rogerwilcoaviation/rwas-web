...
assert.match(workflow, /node scripts\/production-readonly-smoke\.mjs/);
assert.match(workflow, /steps\.cf-deploy-production\.outcome == 'success'/);
assert.doesNotMatch(workflow, /github\.event_name == 'push'|npm run catalog:production-smoke/);
const readonlySmoke = fs.readFileSync(new URL('./production-readonly-smoke.mjs', import.meta.url), 'utf8');
assert.match(readonlySmoke, /method: 'GET'/);
assert.doesNotMatch(readonlySmoke, /method: '(POST|PATCH|DELETE|PUT)'/);
...
console.log(JSON.stringify({ actualCartFunctionBlocked: 3, actualCallGateSkipped: true, networkCalls: calls, cartAttempts: attempted, workflowAlwaysReadOnly: true }));
